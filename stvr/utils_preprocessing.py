...
def  traceset_to_pattern_one_hot(traceset,dataset_name,filepath="./data/",freq=0.1,spmf_bin_location_dir="./stvr/"):
    ...
    # print(textset)
    filename="spmf_dataset_v3_"+dataset_name+"_"+str(freq)+"_v2"
    ...
    for i,trace in enumerate(textset):
        ...
        for j,pattern in enumerate(lst_patterns):
            if pattern_in_sentence(pattern[0],trace):
                lst_encoding[i][j]=1
    filename_npy="spmf_one_hot_"+dataset_name+"_"+str(freq)+"_v2.npy"
    filename_lst="spmf_lst_patterns_"+dataset_name+"_"+str(freq)+"_v2.json"
...
def load_spmf_files(filepath,traceset,dataset_name,freq,spmf_bin_location_dir="./stvr/"):
    filename_npy="spmf_one_hot_"+dataset_name+"_"+str(freq)+"_v2.npy"
    filename_lst="spmf_lst_patterns_"+dataset_name+"_"+str(freq)+"_v2.json"
    ...
    if exists(filepath+filename_npy) and exists(filepath+filename_lst):
        ...
        warning_string="These files do not exists :"+str(filepath)+filename_npy+"|" +str(filepath)+filename_lst+"\n creating them..."
        warnings.warn(warning_string)
        spmf_one_hot,spmf_lst_patterns=traceset_to_pattern_one_hot(traceset,dataset_name,filepath=filepath,freq=freq,spmf_bin_location_dir=spmf_bin_location_dir)
